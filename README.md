...
| :--- | :---: | :--- | :--- |
| **ESP32-S3-WROOM-1U** | 1 | MCU (16MB Flash, 8MB PSRAM, U.FL connector) | N16R8 Version - This is a problem, I wanted to ship it assembled with the pcb from jlcpcb, but unfortunately it must be ordered with standard pcb, that means that the price would rise from aproximately 33 USD to crazy over 100 USD - So my plan is to order it from LSCS.com and ship it with the LSCS parts in the same box, because they support it, I think -  https://www.lcsc.com/product-detail/C3013946.html?s_z=n_esp32-s3-wroom-1u |
